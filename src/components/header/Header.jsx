...
export default function Header()
{
  let [Search,setSearch]= useState([])  
   ...
   return(
    ...
    <div className="xl:flex xl:justify-evenly xs:flex xs:justify-evenly  ">
     ...
     xl:absolute xl:top-2  xl:ml-96 xl:mt-1 xl:h-8 xl:w-16  xl:text-white xl:rounded xl:bg-gray-700 xl: xl:active:underline" >Search</button>
     </div>  
  ...
  <NavLink to="/"  className={({isActive})=>`${isActive?"text-gray-950":"text-black-400"}`}> 
      ...
      <li id='li1' className=" xs:ml-1 xs:active:xs:text-black xs:h-16 xs:mt-4 xs:absolute xs:top-0 xs:hover:underline">
     ...
     <NavLink to="arduino" className={({isActive})=>` ${isActive?"text-gray-950":"text-black-400"}`}> 
      ...
      <li className="xs:ml-16 xl:ml-3 xl:hover:underline xl:hover:text-slate-700" >
